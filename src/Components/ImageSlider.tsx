...
const ImageSlider: React.FC<ImageSliderProps> = (props) => {
    ...
    const NextImage = () => {
        if (Index < props.Pathes.length - 1)
            SetIndex(Index + 1)
        else SetIndex(0)
    }
    ...
    const PreviousImage = () => {
        if (Index > 0)
            SetIndex(Index - 1)
        else SetIndex(props.Pathes.length - 1)
    }
    let res: any;
    let com: any;
    if (props.IsSlider) {
        ...
        res = <div>
            <svg onClick={PreviousImage} xmlns="http://www.w3.org/2000/svg" version="1.1" x="0px" y="0px" width="103.537px" height="103.537px" viewBox="0 0 103.537 103.537" >
            ...
            <svg onClick={NextImage} xmlns="http://www.w3.org/2000/svg" version="1.1" x="0px" y="0px" width="103.536px" height="103.536px" viewBox="0 0 103.536 103.536">
        ...
        com = <img src={props.Pathes[Index]} alt='nop' />;
    }
    else {
        res = undefined;
        com = <iframe title='nop' src={"https://www.youtube.com/embed/" + props.Pathes[0]}></iframe>
    }
    ...
    return (
        <div className='ImageSlider'>
            <div className='ImageSliderContent'>
                {res}
                <div className='ISText'>{props.Title}</div>
                {com}
            </div>
        </div>
    )
